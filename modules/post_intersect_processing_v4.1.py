...
def compare_ind_blocks(nagata_blockSizes,annotate_blockSizes,some_threshold,blockCount):
    ...
    if blockCount > 1:
        ...
        if abs(int(str(nagata_blockSizes).strip(','))-int(str(annotate_blockSizes).strip(','))) < some_threshold:
            ...
            return False
def compare_first(nagata_blockSizes,annotate_blockSizes,first_exon_threshold,strand):
    pass_threshold = []
    # all([True if abs(nagata_block-anno_block) < some_threshold else False  for nagata_block,anno_block in zip(x[1:],y[1:])])

    if strand == "+":
        nagata_first_exon = list(map(int,str(nagata_blockSizes).strip(',').split(',')))[0]
        annotate_first_exon = list(map(int,str(annotate_blockSizes).strip(',').split(',')))[0]
#         print(nagata_first_exon)
#         print(annotate_first_exon)
        return True if abs(nagata_first_exon - annotate_first_exon) < first_exon_threshold else False 
    else:
        nagata_first_exon = list(map(int,str(nagata_blockSizes).strip(',').split(',')))[-1]
        annotate_first_exon = list(map(int,str(annotate_blockSizes).strip(',').split(',')))[-1] 
#         print(nagata_first_exon)
#         print(annotate_first_exon)
        return True if abs(nagata_first_exon - annotate_first_exon) < first_exon_threshold else False 
...
def run_overlap_scoring(output_file,nagata_file,annotation_file,overlap_parameter=50,ind_overlap_parameter=20):
    ...
    bedtools_p1 = f"bedtools intersect -wo -split -s -a {nagata_file} -b {annotation_file}".split(" ")
...
if __name__ == '__main__':
    ...
    requiredGrp.add_argument("-f",'--first_exon_threshold', required=False, help="First exon threshold",default=20,type = float)
...
#         print(rows)
        current_indv = compare_ind_blocks(rows[10],rows[22],ind_overlap_parameter,rows[9])
#         print(current_indv)
        current_first = compare_first(rows[10],rows[22],first_exon_threshold,strand)
#         print(current_first)
        keep_rows.append(current_indv)
        first_exons.append(current_first)
        
    final_df[29] = keep_rows
    final_df[30] = first_exons
    final_df = final_df[final_df[29] == True]
    final_df = final_df[final_df[30] == True]
#     print(final_df.he)
    final_df = final_df.drop([24,25,26,29,30],axis = 1)
